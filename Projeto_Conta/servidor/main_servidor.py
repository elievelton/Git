            ...
            # cadastrar cliente [2, nome1, endereco2, cpf3, nascimento4, usuario5, senha6]
                ...
                if(Busca_conta_de_destino):
                    ...
                    if (buscar_conta[0][0] != None):
                        ...
                        self.ban.transferirBD(
                            conexao, Busca_conta_de_destino[0][0], buscar_conta[0][1], valor)
                        cs1 = self.ban.Buscar_conta_bd(conexao, buscar_conta[0][1])

                        cliente = str(
                            self.ban.Buscar_cliente_bd_login(conexao, cs1[0][1]))
                        ...
                        self.conex.send(
                            ('0, Transferencia realizada com sucesso!,' + resultado).encode())
                    else:
                        self.conex.send(
                ...
                conta = self.ban.Buscar_conta_bd(conexao, valor[0][6])
                convert_conta = concatenar_operacao(conta)
                resultado = lista + convert_conta
                # remove aspas e () das mensagens
                tratamento = replace_dados(resultado)
                ...
                dados = self.ban.lendo_dados(conexao, q1)
                lista = list(dados)
                busca_conta = self.ban.Buscar_conta_bd_login(conexao, login)
